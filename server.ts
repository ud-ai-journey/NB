...
const apiKey = process.env.GEMINI_API_KEY || "AQ.Ab8RN6JekD9-tuikbGFO8q5WXVBOz7u6o3dOwZcVJdfqMWiZiw"; // Use injected key or user's provided fallback
...
async function callAIEngine(
  ...
  try {
    ...
    try {
      ...
      const openAiKey = process.env.OPENAI_API_KEY || "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";
...
app.post("/api/coaching/generate-plan", async (req, res) => {
  try {
    const { habitName, category, currentLevel, targetGoal, triggers } = req.body;
...
Ensure the mitigation strategies are creative, realistic, and tailored to the specific triggers provided. 
...
User's triggers/stress factors: ${triggers || "Boredom, stress"}.
...
The user is working to break/reduce a habit: ${habitContext ? JSON.stringify(habitContext) : "a bad habit"}.
Your tone should be:
...
- Avoid writing long, dry walls of text. Keep responses focused, conversational, and split into readable short paragraphs (max 120-150 words).`;
    ...
    const aiResult = await callAIEngine(systemInstruction, messages, null, 0.7);
